...
def initial_u(N,J,s, r_ghost_pt):
    ...
    # the second row -- have to solve system of equations -- assumes dirichlet left point and neumann right point
    # we don't use the ghost point here, but we do update it afterwards
    alpha = 2*s+1
    beta = -s
    A = np.zeros((J-2,J-1))
    for j in range(J-2):
    ...
    try:
        X, res, rank, singulars = np.linalg.lstsq(A,B)
    except LinAlgError:
        print "No error handling. np.linalg.lstsq() failed to compute second row of solution."
...
def picard_engage(u, s, J, N, r_ghost_pt):
    ...
    for n in range(N-2):
        ...
        u[n+2,J-1] = s*(r_ghost_pt + u[n,J-2]) - (2*s+1)*u[n,J-1] + 2*u[n+1,J-1]
